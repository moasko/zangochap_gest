  ...
  return <div className="mx-auto w-full max-w-[1600px] space-y-4 p-4 lg:p-6">
    ...
    <div className="grid gap-4 lg:grid-cols-[260px_minmax(0,1fr)]">
      <aside className="max-h-[35vh] space-y-2 overflow-y-auto lg:max-h-[65vh]">
        {mode === "live" ? <>{live.isPending && <p className="p-3 text-sm text-slate-500">Chargement des livreurs…</p>}{riders?.filter(r => !riderId || r.id === riderId).map(r => { const state = states?.find(s => s.riderId === r.id); const status = trackingStatus(state, now); return <button key={r.id} onClick={() => setRiderId(r.id)} className="w-full rounded-lg border border-slate-200 bg-white p-3 text-left"><span className="block text-sm font-bold text-slate-900">{r.name}</span><span className={"mt-1 block text-xs font-medium " + (status === "Suivi actif" ? "text-emerald-700" : "text-slate-500")}>{state ? status : "Aucun partage"}</span><span className="mt-2 block text-xs text-slate-500">{state?.capturedAt ? formatTime(state.capturedAt) + " · ±" + Math.round(state.accuracy || 0) + " m" : "Aucune position reçue"}</span></button>; })}{riders?.length === 0 && <p className="p-3 text-sm text-slate-500">Aucun livreur enregistré.</p>}<p className="p-2 text-xs leading-relaxed text-slate-500">Actualisation toutes les 10 s. Un repère gris correspond à un suivi arrêté ou une position ancienne. La précision dépend du téléphone et du signal GPS.</p></> : <div className="rounded-lg border border-slate-200 bg-white p-3 text-sm">
          <h2 className="font-bold">Parcours du jour</h2><p className="mt-2 text-slate-500">{!riderId ? "Choisissez un livreur pour afficher son parcours." : history.isFetching ? "Chargement du parcours…" : points.length ? points.length + " positions affichées" : "Aucune position pour cette période."}</p>
