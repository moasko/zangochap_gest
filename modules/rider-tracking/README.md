...
## Comparaison et suivi de carte — 2026-09-10
- Historique : sélection explicite de 1 à 5 livreurs, paramètre riderId répété. 10 000 points maximum pour un seul livreur ; 2 000 par livreur en comparaison, soit 10 000 au maximum. Totaux/limites présentés individuellement ; réduire les heures si tronqué. Pas de chargement de tous les livreurs par défaut.
- Masquer/afficher chaque tracé sans nouvelle requête. Segmentation par livreur, puis session et trous GPS > 5 min ; aucun trait reliant deux personnes. Lecture chronologique des points visibles avec nom du livreur courant.
- Direct : filtrage actif/ancien/arrêté/en attente/aucun partage, âge de capture affiché. L’absence de GPS ne prouve pas un arrêt physique.
- Suivre sur la carte centre sur la dernière position connue du livreur sélectionné. Le déplacement ou zoom manuel, le recentrage et le changement de mode/filtre arrêtent ce centrage. Le suivi GPS du livreur n’est pas affecté.
- Validation étendue dans test-rider-tracking.mjs : sélection multiple, plafonds équitables, périodes UTC, séparation des tracés et âge des positions. Aucun changement de base nécessaire.
